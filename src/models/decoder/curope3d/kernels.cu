        ...
        // const int N, const int H, const int D )
{
    ...
    // start of (X/Y/Z) part
    const int X = (threadIdx.x < D/3 ? 0 : 1) + (threadIdx.x < 2*D/3 ? 0 : 1);
    const int m = (X*D/3) + (threadIdx.x % Q);   // index of u_(X/Y/Z)
    ...
    /*
    float* shared_cos_sin = shared + D + D/6;
...
void rope_3d_cuda( torch::Tensor tokens, const torch::Tensor pos, const float base, const float fwd ) 
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::kBFloat16, at::kHalf, 
        tokens.type(), "rope_3d_cuda", ([&] {
